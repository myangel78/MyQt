...
{
	//if (m_tmrInitRecordConfig.isActive())
	//{
	//	m_tmrInitRecordConfig.stop();
	//}
}
...
bool RecordSettingDialog::InitCtrl(void)
{
	...
	//list
	m_pchkAllSelect = new QCheckBox("AllCheck", m_pgrpRecordChannel);
	m_pchkShowVisible = new QCheckBox("ShowVisible", m_pgrpRecordChannel);
	m_pchkShowVisible->setVisible(false);

	m_plistSelect = new QListWidget(m_pgrpRecordChannel);
	m_vetChannelItem.resize(CHANNEL_NUM);
	m_vetChannelSelect.resize(CHANNEL_NUM);
	for (int i = 0; i < CHANNEL_NUM; ++i)
	{
		QListWidgetItem* item = new QListWidgetItem(m_plistSelect);
		m_vetChannelItem[i] = std::make_shared<QCheckBox>(QString("CH %1").arg(i + 1), m_plistSelect);
		m_plistSelect->setItemWidget(item, m_vetChannelItem[i].get());

		m_vetChannelSelect[i] = false;
		connect(m_vetChannelItem[i].get(), &QCheckBox::stateChanged, this, &RecordSettingDialog::onListItemChangeSlot);
	}
	//m_plistSelect->setMinimumWidth(130);
	m_plistSelect->setMaximumWidth(150);

	m_pbtnSynchronize = new QPushButton("SynchronizeChannel", m_pgrpRecordChannel);


	QHBoxLayout* horlayout6 = new QHBoxLayout();
	horlayout6->addWidget(m_pchkAllSelect);
	horlayout6->addWidget(m_pchkShowVisible);

	QVBoxLayout* verlayout6 = new QVBoxLayout();
	verlayout6->addLayout(horlayout6);
	verlayout6->addWidget(m_plistSelect);
	verlayout6->addWidget(m_pbtnSynchronize);

	m_pgrpRecordChannel->setLayout(verlayout6);
	m_pgrpRecordChannel->setVisible(false);
	...
	connect(m_pchkAllSelect, &QCheckBox::stateChanged, this, &RecordSettingDialog::onCheckAllSelect);
...
{
	std::string strjson = ConfigServer::GetUserPath() + "/etc/Conf/experiment_configs.json";
...
bool RecordSettingDialog::Json4SaveSetting(const std::string& spath, const std::string& sname)
{
	...
	QString hardver = (ConfigServer::GetInstance()->GetHardwareAmpBoardVer() == 1) ? "V1_" : "V2_";
...
{
	std::string strjson = ConfigServer::GetUserPath() + "/etc/Conf/model_configs.json";
	if (!std::filesystem::exists(strjson))
	{
		return false;
	}
...
bool RecordSettingDialog::UpdateChannelSelected(void)
{
	for (int i = 0; i < m_pControlView->m_iStartIndex; ++i)
	{
		m_plistSelect->item(i)->setHidden(true);
		m_vetChannelItem[i]->setChecked(false);
		m_vetChannelSelect[i] = false;
	}
	for (int i = m_pControlView->m_iEndIndex; i < CHANNEL_NUM; ++i)
	{
		m_plistSelect->item(i)->setHidden(true);
		m_vetChannelItem[i]->setChecked(false);
		m_vetChannelSelect[i] = false;
	}
	for (int i = m_pControlView->m_iStartIndex; i < m_pControlView->m_iEndIndex; ++i)
	{
		m_plistSelect->item(i)->setHidden(m_pControlView->m_plistControlShow->item(i)->isHidden());
		bool chk = m_pControlView->m_vetControlListItem[i]->GetCheckedSeq();
...
void RecordSettingDialog::onCheckAllSelect(int val)
{
	for (int i = 0; i < CHANNEL_NUM; ++i)
	{
		bool chk = (val != Qt::CheckState::Unchecked);
		m_vetChannelItem[i]->setChecked(chk);
		m_vetChannelSelect[i] = chk;
	}
}

void RecordSettingDialog::onCheckShowVisible(int val)
{
	if (val != Qt::CheckState::Unchecked)
	{
		for (int i = 0; i < CHANNEL_NUM; ++i)
		{
			m_plistSelect->item(i)->setHidden(!m_vetChannelItem[i]->isChecked());
		}
	}
	else
	{
		for (int i = 0; i < CHANNEL_NUM; ++i)
		{
			m_plistSelect->item(i)->setHidden(false);
		}
	}
}

void RecordSettingDialog::onListItemChangeSlot(int val)
{
	for (int i = 0; i < CHANNEL_NUM; ++i)
	{
		m_vetChannelSelect[i] = m_vetChannelItem[i]->isChecked();
	}
}

void RecordSettingDialog::onClickSynchronize(void)
{

...
TemperatureMonitor::TemperatureMonitor(QWidget* parent) : QDialog(parent)
{
...
void TemperatureMonitor::InitPortCtrl(void)
{
    ...
    connect(m_pPortConnectBtn, SIGNAL(clicked()), this, SLOT(OnPortConnectBtnSlot()));
...
void TemperatureMonitor::InitTempDispView(void)
{
    ...
    m_pTempCurveStartBtn = new QPushButton(QString::fromLocal8Bit("开始"), m_pTempCtrlGpbox);
    m_pTempCurveStartBtn->setCheckable(true);
    m_pTempCurveStartBtn->setIcon(QIcon(":img/img/start.ico"));
    ...
    thorlayout->addWidget(m_pTempCurveStartBtn);
    ...
    //connect(m_pTempCurveStartBtn, &QPushButton::toggled, [&](bool chk) { \
    //    m_pTempCurveStartBtn->setText(chk ? QString::fromLocal8Bit("暂停") : QString::fromLocal8Bit("开始")); m_bNeedUpdate = chk; });
    connect(m_pTempCurveStartBtn, &QPushButton::clicked, this, &TemperatureMonitor::OnClickStartUpdateData);
...
{
    m_pSerialPort->Close();
    m_pSearchPortComb->setEnabled(true); //
    m_pPortBaudRateComb->setEnabled(true);
}

/**
 * @brief 连接串口
 */
void TemperatureMonitor::OnPortConnectBtnSlot(void)
{
...
void TemperatureMonitor::OnPortSettingBtnSlot(void)
{
    if (m_pPortSettingDialog)
    {
        ...
        ConfigServer::WidgetPosAdjustByParent(this, m_pPortSettingDialog);
    }
}
...
bool TemperatureMonitor::OpenPort(void)
{
    ...
    {
        m_pSearchPortComb->setEnabled(false); // 禁止更改串口
        m_pPortBaudRateComb->setEnabled(false);
        return true;
    }
...
{
    m_pTempCurDispLcd->display(curTemp);
    m_pTempCurTargetDescrLcd->display(curTargetTemp);
    emit SetCurrentTempSignal(curTemp);
}
...
void TemperatureMonitor::OnClickStartUpdateData(bool chk)
{
    if (chk)
    {
        m_pTempCurveStartBtn->setText(QString::fromLocal8Bit("暂停"));
        m_pTempCurveStartBtn->setIcon(QIcon(":img/img/pause.ico"));
    }
    else
    {
        m_pTempCurveStartBtn->setText(QString::fromLocal8Bit("开始"));
        m_pTempCurveStartBtn->setIcon(QIcon(":img/img/start.ico"));
    }
    m_bNeedUpdate = chk;
}
...
void TemperatureMonitor::OnSwichFunctViewChange(int index)
{
    ...
    if (index == TEMP_VIEW_CURVE_INDEX)
    {
        m_pViewStackWidget->setCurrentIndex(TEMP_VIEW_CURVE_INDEX);
    }

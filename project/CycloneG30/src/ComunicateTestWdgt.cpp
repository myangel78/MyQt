...
void ComunicateTestWdgt::InitCtrl(void)
{
    ...
    gridrcvlayout->addWidget(m_pReadButton,2,1,1,1);
    gridrcvlayout->addWidget(m_pRcvClearBtn,1,1,1,1);
...
void ComunicateTestWdgt::OnSendBtnSlot(void)
{
    ...
    if(m_pUsbFtdDevice->WriteData(acWriteBuf,ulBytesToWrite,&ulActualBytesToWrite))
    {
         qDebug("\n>>>>>>>>>>>>>>>>>>>%d senddata actual %d ",ulBytesToWrite,ulActualBytesToWrite);
...
//        if(!data.isEmpty())
//        {
//            m_pRcvTextEdit->append(QString::fromStdString(data.toStdString()));
//            m_pRcvTextEdit->moveCursor(QTextCursor::End);
//        }
    }

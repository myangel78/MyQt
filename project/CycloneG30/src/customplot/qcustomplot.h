...
public:
  ...
  void preReserve(const int &size){mData.reserve(size);}
...
public:
  ...
  */
  ...
  // non-property methods:
  void addData(QVector<QCPGraphData> &&vctData);
  void addData(const QVector<double> &keys, const QVector<double> &values, bool alreadySorted=false);
  void addData(double key, double value);
  void addData(const QVector<QCPGraphData> &tempData, bool alreadySorted=false);

  //pre allocate vector size
  void preReserve(const int& size) { mDataContainer->preReserve(size);  }
  ...
  bool GetLastDataRange(void) const { return m_bSetLastDataRange; }
  void SetLastDataRange(bool val) { m_bSetLastDataRange = val; }
...
protected:
  ...
  bool m_bSetLastDataRange = true;
  int m_lastLineDataRangeEnd = -1;

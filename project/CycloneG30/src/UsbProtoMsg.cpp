...
//    LOGCI(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>{0} {1:d}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<",__FUNCTION__, frame);
...
//            LOGCI("ptr[0]:{0:x},ptr[1]:{1:x},ptr[2]:{2:x},ptr[3]:{3:x}",ptr[0],ptr[1],ptr[2],ptr[3]);
        }
        else if(flag == 1)
        {
            flag = 0;
            ...
            if(crcCalRes == crcExtract )
                LOGCI("channel {0} extract CRC from data: {1:x}, Calculate CRC result: {2:x}",channel,crcExtract,crcCalRes);
            else
                LOGCE("channel {0} extract CRC from data: {1:x}, Calculate CRC result: {2:x}",channel,crcExtract,crcCalRes);
#else
            if(crcCalRes != crcExtract )
                LOGCE("channel {0} extract CRC from data: {1:x}, Calculate CRC result: {2:x}",channel,crcExtract,crcCalRes);
#endif
